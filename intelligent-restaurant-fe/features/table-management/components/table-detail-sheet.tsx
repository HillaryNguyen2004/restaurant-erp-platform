...
export function TableDetailSheet({ table, onClose, defaultTab }: Props) {
  ...
  const canPlaceOrders =
    role === "TABLE" || role === "TABLE_STAFF" || role === "ADMIN"
  ...
  const placeMutation = useMutation({
    ...
    onSuccess: async (newOrder) => {
      // In real mode, order-menu publishes to Kafka and kitchen-operation creates the ticket.
  ...
  const checkoutMutation = useMutation({
    mutationFn: async () => {
      await tableApi.checkoutTable(table!.tableNumber)
      await tableApi.updateTableStatus(table!.id, "AVAILABLE")
    },
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["table-orders", table?.tableNumber],
