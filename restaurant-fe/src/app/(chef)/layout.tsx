        ...
        <div className="min-h-screen flex flex-col bg-slate-950 text-slate-100">
            <header className="h-20 border-b border-slate-800 bg-slate-900/50 backdrop-blur-sm px-6 flex items-center justify-between sticky top-0 z-50">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-xl font-black tracking-widest text-white uppercase italic">KDS Central</h1>
                        <p className="text-xs text-slate-400 font-medium">Bếp - Đang hoạt động</p>
                    </div>
                </div>

                <nav className="flex items-center gap-2">
                    <div className="hidden md:flex items-center bg-slate-800 rounded-lg p-1 mr-4">
                        <Button variant="secondary" className="gap-2 bg-slate-950 text-rose-500 hover:text-rose-400">
                            <Timer className="w-4 h-4" /> Đang chế biến (8)
                        </Button>
                        <Button variant="ghost" className="gap-2 text-slate-400 hover:text-white">
                            <History className="w-4 h-4" /> Đã hoàn tất
                        </Button>
                    </div>
                    <Button variant="ghost" size="icon" className="text-slate-400 hover:text-white">
                        <Settings className="w-5 h-5" />
                    </Button>
                    <Button variant="ghost" size="icon" className="text-rose-500 hover:bg-rose-950/30" onClick={() => logout()}>
