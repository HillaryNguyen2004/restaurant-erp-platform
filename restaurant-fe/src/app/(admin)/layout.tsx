        ...
        <div className="min-h-screen flex bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100">
            ...
            <aside className="w-20 lg:w-72 flex flex-col border-r bg-white dark:bg-slate-900 sticky top-0 h-screen transition-all duration-300 group">
                <div className="h-16 flex items-center px-6 border-b">
                    <div className="w-8 h-8 rounded bg-indigo-600 flex-shrink-0 flex items-center justify-center font-bold text-white">A</div>
                    <h1 className="ml-3 font-bold text-xl hidden lg:block overflow-hidden transition-all uppercase tracking-tight">Admin Console</h1>
                </div>
                <nav className="flex-1 py-6 px-4 space-y-1">
                    {[
                        { icon: LayoutDashboard, label: "Dashboard", role: "all" },
                        { icon: MenuIcon, label: "Quản lý Menu", role: "all" },
                        { icon: Package, label: "Kho hàng", role: "all" },
                        { icon: Users, label: "Nhân viên", role: "all" },
                        { icon: BarChart3, label: "Báo cáo", role: "all" },
                        { icon: Settings, label: "Cài đặt", role: "all" },
                    ].map((item, idx) => (
                        <Button
                            key={idx}
                            variant="ghost"
                            className={cn(
                                ...
                                idx === 0 && "bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600"
                            )}
                        >
            ...
            <div className="flex-1 flex flex-col min-w-0">
                <header className="h-16 bg-white dark:bg-slate-900 border-b flex items-center justify-between px-8 sticky top-0 z-30">
                    <div className="flex items-center gap-2 text-slate-500 text-sm">
                        ...
                        <span className="text-slate-900 dark:text-slate-200 font-medium italic">Overview</span>
                    </div>
                    <div className="flex items-center gap-4">
                        <div className="text-right hidden sm:block">
                            <p className="text-sm font-bold">{user.email}</p>
                            <p className="text-[10px] text-indigo-500 font-semibold uppercase tracking-widest">Business Owner</p>
                        </div>
