...
public class OrderUpdater {
    ...
    public Order updateItem(UUID sessionId, UUID orderId, UUID itemId, UpdateOrderItemRequest request) {
        ...
        order.updateOrderItem(itemId, request.quantity(), request.modifiers(), request.specialInstructions());
        Order saved = orderRepository.save(order);
        orderEventPublisher.publishOrderItemUpdated(saved, itemId);
        return saved;
    }
}
